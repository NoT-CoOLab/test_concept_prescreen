...
5. **Every single response is saved locally in the browser immediately** (so refreshing or closing the tab loses nothing on that device). It is possible to turn on `emailEnabled` to email their responses to the researcher. Otherwise, responses can be downloaded at the end. Currently, `emailEnabled` is on.
